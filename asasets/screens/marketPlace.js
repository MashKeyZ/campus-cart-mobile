...
import socket from '../socket';
 ...
 const MarketPlace = ({token,gProducts,gCart,setGlobalCart,currentUser,profile}) => {
  const [products,setProducts]= useState(gProducts)
  const [isLoading,setLoading] = useState(false)
  const [cart,setCart] = useState(gCart)
  ...
  async function loadProducts(){
    ...
    try {
      ...
      const response = await axios.get(`${serverURL}/products/`, config);
  ...
  async function search(name){
    ...
    try {
      ...
      const response = await axios.post(`${serverURL}/search/search`,{name:name}, config);
      ...
      if(response.status==200){
        ...
        //setProducts([...products, ...updatedProducts])
        setDisplayList(updatedProducts)
