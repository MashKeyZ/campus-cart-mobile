...
const FULL_HEIGHT = Dimensions.get('window').height;
 const MyProfile = ({gCart,token,socket,currentUser,profile}) => {
  console.log(gCart)
  let  registrationData  = '';
    const path = require('../images/image.png')
    const [imagePath, setImagePath] = useState(profile.image)
    const [isLoading,setLoading] = useState(false)
    const [errorMessage,setErrorMassege] = useState('')
    const [error,setError] = useState(false)
    const profileName = profile.firstName.split(' ')[0]+' '+profile.lastName.split(' ')[0];
    ...
    const saveChanges =async()=>{
      ...
      try {
        ...
        const response = await axios.post(`${serverURL}/users/changeprofile`,
        {data:{
          imagePath:imagePath,
        }} 
        ,config);
    ...
    const navToMyProducts=()=>{
      console.log('CurrentuserId: '+currentUser)
      navigation.navigate('MyProducts',{token:token,currentUser:currentUser})
    }
   ...
   return (
    <SafeAreaView style={styles.container}>
        ...
        <View style={styles.footer}>
            ...
            <TouchableOpacity style={styles.navButton} onPress={navToMyProducts}>
                <Text style={{
                    color:'#FFFFFF',
                    fontSize:24,
                    fontWeight:'500'
