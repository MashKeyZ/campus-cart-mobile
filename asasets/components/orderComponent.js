...
import React,{useEffect, useState} from 'react';
 ...
 const categoryList = [
  ...
  useEffect(()=>{
    if(category!==''){
        setOrderStatus(category)
        const text ={orderId:item?.orderId,orderStatus:category,sendTo:item?.fromUserId}
        
        socket.emit('orderchange',JSON.stringify(text))
    }
    setCategory(prev=>'');
        ...
        <View style={styles.rightCol}>
          ...
          <View style={styles.textInputDrop}>
              <DropDown
                  ...
                  list={categoryList}
                  activeOutlineColor="rgba(66, 133, 244, 0.5)"
                  outlineColor="#FFFFFF"
